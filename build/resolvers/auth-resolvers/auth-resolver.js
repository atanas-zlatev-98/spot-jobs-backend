...
let AuthResolver = class AuthResolver {
    async currentUser(ctx) {
        if (!ctx.user) {
            throw new apollo_server_core_1.AuthenticationError('User is not authenticated!');
        }
    ...
    async login({ email, password }) {
        ...
        if (!user) {
            throw new apollo_server_core_1.UserInputError('Wrong Login Credentials');
        }
        ...
        await user.save();
        return (0, token_1.getToken)(user._id);
    }
};
__decorate([
...
__decorate([
    ...
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [login_arguments_1.UserLoginArguments]),
    __metadata("design:returntype", Promise)
